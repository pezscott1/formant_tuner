...
def vowel_window(voice_type: str, vowel: str, calibrated: dict | None = None):
    """
    Returns (lo1, hi1, lo2, hi2) for F1/F2 windows.

    Behavior expected by tests:
      - If calibrated[vowel] has f1,f2 and f0 not None → scaled windows
      - If calibrated invalid/missing → use VOWEL_CENTERS fallback
      - If voice_type unknown → fall back to "tenor"
      - If vowel unknown even in fallback → return None
    """
    # 1. Try calibrated
    if calibrated is not None and vowel in calibrated:
    ...
    # 2. Fallback to table-based centers
    ...
    # If not in main table, try our local default windows as a last resort
    if entry is None and vt in _DEFAULT_WINDOWS:
        default_win = _DEFAULT_WINDOWS[vt].get(vowel)
        if default_win is not None:
            return default_win

    # If we still don't have an entry, signal "no window" to callers
    if entry is None:
        return None
