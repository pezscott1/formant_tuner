...
def test_extract_formants_basic(pm):
    ...
    assert out["a"] == (500, 1500, 100, 0.9, 5)
    assert out["b"] == (400, 1200, 90, 0.0, float("inf"))
    assert "voice_type" not in out
...
def test_apply_profile(pm, tmpdir_profiles, analyzer):
    ...
    assert analyzer.calibrated_profile["a"][0] == 500
    assert analyzer.set_user_formants_called
    assert analyzer.reset_called
